package yibo.zhang.spider.spider1.QQwry;
...
public class IPSeeker {
	...
	// 保存的文件夹
	private String INSTALL_DIR = "e:\\qqwry.dat";
	// 一些固定常量，比如记录长度等
	private static final int IP_RECORD_LENGTH = 7;
	private static final byte REDIRECT_MODE_1 = 0x01;
	private static final byte REDIRECT_MODE_2 = 0x02;

	// 用来作为cache，查询一个IP时首先查看cache，以减少不必要的重复查找
	...
	public IPSeeker(String fileName, String dir) {
		...
		try {
			...
			// 如果我找不到这个文件，再尝试在当前目录下搜索：这次全部改用小写文件名
					...
					if (files[i].getName().toLowerCase().equals(filename)) {
						try {
							ipFile = new RandomAccessFile(files[i], "r");
							 ...
							 LogFactory.log("IP地址信息文件没有找到，IP显示功能将无法使用",Level.WARNING,el);
							ipFile = null;
						}
						break;
					}
		...
		if (ipFile != null) {
			try {
				...
				if (ipBegin == -1 || ipEnd == -1) {
				 ...
				 LogFactory.log("IP地址信息文件没有找到，IP显示功能将无法使用",Level.WARNING,e);
				ipFile = null;
			}
		}
	...
	/**给定一个地点的不完全名字，得到一系列包含s字串的IP范围记录
	 * @param s 地点子串
	 * @return 包含IPEntity类型的List
	 */
	public List getIPEntriesDebug(String s){
		...
		for(long offset=ipBegin+4;offset<=endOffset;offset+=IP_RECORD_LENGTH){
			...
			if(temp!=-1){
				...
				//判断这个地点里面是否包含s子串，如果包含，添加这个记录到List中；
				//如果没有，继续
	...
	public List<IPEntry> getIPEntries(String s){
		...
		try{
			...
			for(int offset=(int)ipBegin+4;offset<=endOffset;offset+=IP_RECORD_LENGTH){
				...
				if(temp!=-1){
					IPLocation ipLoc=getIPLocation(temp);
					//判断这个地点里面是否包含s子串，如果包含，在List中添加这个记录；
					//如果没有，继续
					if(ipLoc.getCountry().indexOf(s)!=-1||ipLoc.getArea().indexOf(s)!=-1){
						IPEntry entry=new IPEntry();
						entry.country=ipLoc.getCountry();
				 ...
				 LogFactory.log("",Level.WARNING,e);
			}
			return ret;
	...
	/**
	 * 从内存映射文件offset位置开始的第3个字节起读取一个int
	 * 
	 * @param offset
	 * @ return/
	 */
	...
	public String getCountry(byte[] ip) {
		...
		//先检查cache中是否已经包含这个IP的结果，没有再搜索文件
	...
	/**
	 * 从offset位置读取4字节的IP地址放入ip数组中，读取后的ip为Big-endian格式，但是文件中是little-endian形式，将会进行转换
	 */
	...
	/**
	 * 把类成员ip和beginIp比较，注意这个beginIp是Big-endian格式的
	 * 
	 * @param ip要查询的ip
	 * @param beginIP和被查询IP相比较的IP
	 * @return 若ip和begin相等返回0，ip大于beginIp返回1，小于则返回-1
	 */
	...
	/**
	 * 这个方法根据IP的内容，定位到包含这个IP国家地区的记录处，返回一个绝对偏移 使用二分法查找
	 * 
	 * @param ip
	 *            要查询的IP
	...
	private long locateIP(byte[] ip) {
		...
		//如果循环结束，那么i和j必定是相等的。这个记录为最可能的记录，但是并非
	...
	private IPLocation getIPLocation(long offset){
	  ...
	  /**
	   * 给定一个IP国家地区记录的偏移，返回一个IPLocation结构，此方法应用于内存映射文件方式
	   * @param offset 国家记录的起始偏移
	   * @return IPLocation对象
	   */
